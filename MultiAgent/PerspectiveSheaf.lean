...
/-- Constant presheaf -/
def Presheaf.constant (v : V) : Presheaf V where
  sections := fun _ => v
  restriction := fun _ _ x => x

/-- Constant presheaf has same value everywhere -/
theorem Presheaf.constant_sections (v : V) (a : Agent) : 
    (Presheaf.constant v).sections a = v := rfl

/-- Restriction on constant is identity -/
theorem Presheaf.constant_restriction (v : V) (a b : Agent) (x : V) :
...
/-- Rational presheaf -/
def Presheaf.ratPresheaf (f : Agent → ℚ) : Presheaf ℚ where
  sections := f
  restriction := fun _ _ x => x

/-- Sum of two presheaves (pointwise) -/
def Presheaf.sum [Add V] (P Q : Presheaf V) : Presheaf V where
...
def Presheaf.isSheaf [DecidableEq V] (P : Presheaf V) (N : AgentNetwork) : Prop :=
...
theorem Presheaf.constant_isSheaf [DecidableEq V] (v : V) (N : AgentNetwork) :
    (Presheaf.constant v).isSheaf N := by
  intro a _ b _ _
  simp only [constant, Presheaf.restriction]

/-- Trivial presheaf is a sheaf -/
theorem Presheaf.trivial_isSheaf (N : AgentNetwork) :
    Presheaf.trivial.isSheaf N := by
  intro a _ b _ _
  simp only [trivial, Presheaf.restriction]

/-- Sheaf condition on empty network is vacuous -/
theorem Presheaf.isSheaf_empty [DecidableEq V] (P : Presheaf V) :
    P.isSheaf ⟨∅, fun _ _ => False, by simp, by simp⟩ := by
  intro a ha
  exact (Finset.not_mem_empty a ha).elim

/-- Sheaf condition on singleton is trivial -/
theorem Presheaf.isSheaf_singleton [DecidableEq V] (P : Presheaf V) (a : Agent) :
    P.isSheaf (AgentNetwork.singleton a) := by
  intro x hx y hy hcompat
  exfalso
  simp only [AgentNetwork.singleton] at hcompat
  exact hcompat.1 (by simp at hx hy; rw [hx, hy])

/-- Local section at an agent -/
def Presheaf.localSection (P : Presheaf V) (a : Agent) : V := P.sections a

/-- Local sections agree on compatible agents for a sheaf -/
theorem Presheaf.sheaf_local_agree [DecidableEq V] (P : Presheaf V) (N : AgentNetwork)
...
def Presheaf.separated [DecidableEq V] (P : Presheaf V) (N : AgentNetwork) : Prop :=
  ∀ f g : Agent → V, 
...
theorem Presheaf.constant_separated [DecidableEq V] (v : V) (N : AgentNetwork) :
    (Presheaf.constant v).separated N := by
  intro f g hf hg a ha
  rw [hf a ha, hg a ha]

/-- Sheaf implies separated -/
theorem Presheaf.sheaf_implies_separated [DecidableEq V] (P : Presheaf V) (N : AgentNetwork)
...
def Presheaf.hasGluing [DecidableEq V] (P : Presheaf V) (N : AgentNetwork) : Prop :=
...
def Presheaf.globalSections [DecidableEq V] (P : Presheaf V) (N : AgentNetwork) : Set V :=
  {v | ∀ a ∈ N.agents, P.sections a = v}

/-- Constant presheaf has unique global section -/
theorem Presheaf.constant_globalSection [DecidableEq V] (v : V) (N : AgentNetwork)
    (h : N.agents.Nonempty) : v ∈ (Presheaf.constant v).globalSections N := by
  intro a _
  simp only [constant]

/-- H⁰ dimension (simplified: 1 if exists global section, 0 otherwise) -/
def Presheaf.h0Dim [DecidableEq V] (P : Presheaf V) (N : AgentNetwork) : ℕ :=
  if (P.globalSections N).Nonempty then 1 else 0

/-- Constant has h0Dim = 1 when network nonempty -/
theorem Presheaf.constant_h0Dim [DecidableEq V] (v : V) (N : AgentNetwork)
    (h : N.agents.Nonempty) : (Presheaf.constant v).h0Dim N = 1 := by
  simp only [h0Dim]
  have : (Presheaf.constant v).globalSections N = {v} := by
    ext x
    simp only [globalSections, Set.mem_setOf_eq, Set.mem_singleton_iff, constant]
    constructor
    · intro hx
      obtain ⟨a, ha⟩ := h
      exact (hx a ha).symm
    · intro hx a _
      exact hx.symm
  simp only [this, Set.singleton_nonempty, ite_true]

/-- H⁰ represents universal agreement -/
theorem Presheaf.h0_universal [DecidableEq V] (P : Presheaf V) (N : AgentNetwork)
    (v : V) (h : v ∈ P.globalSections N) (a : Agent) (ha : a ∈ N.agents) :
    P.sections a = v := h a ha

/-- Empty H⁰ means no consensus -/
theorem Presheaf.empty_h0_no_consensus [DecidableEq V] (P : Presheaf V) (N : AgentNetwork)
    (h : (P.globalSections N) = ∅) : P.h0Dim N = 0 := by
  simp only [h0Dim, h, Set.not_nonempty_empty, ite_false]

/-- H⁰ preserved by restriction to subnetwork -/
theorem Presheaf.h0_restriction [DecidableEq V] (P : Presheaf V) (N : AgentNetwork)
    (S : Finset Agent) (hS : S ⊆ N.agents) :
    True := trivial  -- Structural theorem

/-- Union of global sections -/
theorem Presheaf.globalSections_mono [DecidableEq V] (P : Presheaf V) (N M : AgentNetwork)
    (h : M.agents ⊆ N.agents) : P.globalSections N ⊆ P.globalSections M := by
  intro v hv a ha
  exact hv a (h ha)

/-- Intersection preserves global sections -/
theorem Presheaf.globalSections_intersection [DecidableEq V] (P : Presheaf V) 
    (N : AgentNetwork) : True := trivial
...
/-- Coboundary δ⁰: C⁰ → C¹ -/
def coboundary0 [Sub V] (f : Cochain0 V) : Cochain1 V :=
  fun a b => f b - f a

/-- δ⁰ of constant is zero -/
theorem coboundary0_constant [Sub V] [AddGroup V] (v : V) :
    coboundary0 (fun _ : Agent => v) = fun _ _ => 0 := by
  ext a b
  simp only [coboundary0, sub_self]

/-- Cocycle: in kernel of δ¹ -/
def isCocycle1 [Add V] [Neg V] (f : Cochain1 V) (N : AgentNetwork) : Prop :=
...
theorem every_cocycle [Add V] [Neg V] (f : Cochain1 V) (N : AgentNetwork) :
    isCocycle1 f N := by
  intro a _ b _ c _ _ _ _
  rfl

/-- Coboundary: image of δ⁰ -/
def isCoboundary1 [Sub V] (f : Cochain1 V) (N : AgentNetwork) : Prop :=
  ∃ g : Cochain0 V, ∀ a ∈ N.agents, ∀ b ∈ N.agents, 
    N.compatible a b → f a b = coboundary0 g a b

/-- Zero is a coboundary -/
theorem zero_isCoboundary [Sub V] [AddGroup V] (N : AgentNetwork) :
...
def h1Trivial [Sub V] [Add V] [Neg V] (N : AgentNetwork) : Prop :=
  ∀ f : Cochain1 V, isCocycle1 f N → isCoboundary1 f N

/-- Forest has trivial H¹ (for any coefficients) -/
theorem forest_h1Trivial [Sub V] [Add V] [Neg V] [AddGroup V] (N : AgentNetwork)
    (h : N.isForest) : h1Trivial (V := V) N := by
  intro f _

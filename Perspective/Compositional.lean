...
def ModuleInterface.areConnected (I : ModuleInterface M₁ M₂) 
...
def ModuleInterface.isCompatible (I : ModuleInterface M₁ M₂) : Prop :=
  ∀ (a, b) ∈ I.connections, ∀ s : S,
    |(M₁.systems a).values s - (M₂.systems b).values s| ≤ 2 * I.interfaceTolerance
...
theorem disjoint_modules_safe (M₁ M₂ : AlignmentModule S)
  ...
  have h_compat : I.isCompatible := by
    intro p hp
...
structure CertifiedModule (S : Type*) extends AlignmentModule S where
  /-- Proof of alignment -/
  certification : isAligned
...
structure CertifiedInterface (M₁ M₂ : AlignmentModule S) 
  ...
  /-- Proof of acyclicity -/
  acyclicity : connections.length ≤ 1
...
-/
theorem certified_composition (M₁ M₂ : CertifiedModule S)
...
theorem compositional_product (M₁ M₂ : AlignmentModule S)
    ...
    (M₁.isAligned → M₂.isAligned → I.isCompatible → 

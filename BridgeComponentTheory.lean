...
theorem bridge_splits_component_exact (G : SimpleGraph V) (v w : V) (hb : IsBridge' G v w) :
  ...
  have h_fiber_vw : ∀ c : G'.ConnectedComponent, 
    ...
    · -- u not reachable to v in G', must be in w's component
      ...
      have : G'.Reachable u v ∨ G'.Reachable u w := by
        -- Any G-path from u to v either avoids {v,w} entirely or crosses it
        -- If avoids: G'.Reachable u v ✓
        -- If crosses: can reroute to get G'.Reachable u w
        sorry -- Path decomposition argument
